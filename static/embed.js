(function () {
  ...
  class GlobalInstance {
    ...
    static isInitialized() {
      if (__GOZEN_NOTIFY__?.isInitialized) return true;
      return false;
    }
    static register(params) {
      const oldValue =
        typeof __GOZEN_NOTIFY__ === 'object' ? __GOZEN_NOTIFY__ : {};
      const gozenNotify = {
        ...oldValue,
        ...params,
        isInitialized: true,
    ...
    static getNotificationPopupType() {
      ...
      const position =
        config?.settings?.push_notification?.popups?.requestPopup?.appearance
          ?.contentPosition?.[device] ?? 'bottomLeft';
      const popupType =
        config?.settings?.push_notification?.popups?.requestPopup?.popupType?.[
          device
        ];
    ...
    static getBlockOverRideType() {
      ...
      if (!config) throw new Error('No config found.');
      const position =
        config?.settings?.push_notification?.popups?.blockOverride.location;
      return { position };
    }
  ...
  class CampaignService {
    static getCampaignIdFromEmbeddedTag() {
      const gozenScriptTags = document.querySelectorAll(
        Config.gozenScriptTagSelector
      );
      if (!gozenScriptTags || gozenScriptTags.length === 0)
        throw this.presentCampaignAndScriptVersion(
          [],
          'Your GoZen Notify configuration is missing campaign details.'
        );
      ...
      [...gozenScriptTags].forEach((tag) => {
        ...
        tagSummery.push({
          Script: tag.src,
          'Campaign Id': tag.getAttribute(Config.campaignIdAttribute),
        });
      });
      if (campaignSet.size > 1)
        throw this.presentCampaignAndScriptVersion(
          tagSummery,
          'You have embedded multiple GoZen Notify campaigns in your site.'
        );
      if (srcSet.size > 1)
        throw this.presentCampaignAndScriptVersion(
          tagSummery,
          'You have embedded multiple versions of GoZen Notify campaign tags in your site.'
        );
      const campaignId = gozenScriptTags[0].getAttribute(
        Config.campaignIdAttribute
      );
      if (
        !campaignId ||
  ...
  const getAjax = function (url, cb) {
    ...
    if (!request) throw 'unable to call server';
  ...
  function getReferrer() {
    ...
    try {
      parent &&
        ...
        (referrer = parent.document.location.href);
    } catch (o) {
  ...
  class Logger {
    ...
    static log(message, ...optionalParams) {
      if (this.isDebuggingEnabled()) console.log(message, ...optionalParams);
    }
    static warn(message, ...optionalParams) {
      if (this.isDebuggingEnabled()) console.warn(message, ...optionalParams);
    }
    static error(message, ...optionalParams) {
      if (this.isDebuggingEnabled()) console.error(message, ...optionalParams);
    }
    static table(tabularData, properties) {
      if (this.isDebuggingEnabled()) console.table(tabularData, properties);
    }
  ...
  const serviceWorkerRegistration = async () => {
    try {
      ...
      if (serviceWorker) return serviceWorker.update();
      const serviceWorkerRegistration = await registerSW();
      listenForStateChanges(
        serviceWorkerRegistration,
        serviceWorkerStateChangeCallBack
      );
  ...
  function isServiceWorkerActive(event) {
    return new Promise((resolve, reject) => {
      ...
      reject(
        `Service worker is not active yet current state : ${event?.target?.state}`
      );
    });
  }
  const subscribeToPush = async () => {
    try {
      const serviceWorker = await getSW();
      if (!serviceWorker) throw 'No service worker found!';
      const VAPID_PUBLIC_KEY = localStorage.getItem('vapidPublicKey');
      if (!VAPID_PUBLIC_KEY) throw 'Vapid public key not found';
      ...
      if (local_tenant_id == null || local_lead_id == null) {
        ...
        updateGzLead({
          vapid_config: subscription.toJSON(),
          tenant_id: tenant_id,
          lead_id: lead_id,
        });
      }
  ...
  const updateSubscriber = async () => {
    try {
      ...
      if (
        ...
        JSON.stringify(localVapidConfigKey)
      )
        return;
      const VAPID_PUBLIC_KEY = localStorage.getItem('vapidPublicKey');
      if (!VAPID_PUBLIC_KEY) throw 'Vapid public key not found';
  ...
  function registerServiceWorker() {
    serviceWorkerRegistration()
      .then((serviceWorkerRegistration) =>
        Logger.log(serviceWorkerRegistration)
      )
  ...
  function createAndAppendIframe(params) {
    ...
    const IFRAME_STYLE =
      'position:fixed;z-index:9999999;right:20px;bottom:20px;border:none;display:none;' +
      styles;
  ...
  async function runPushNotificationCampaign(options) {
    try {
      ...
      if (!options)
        throw new Error(
          'Push notification settings are missing in the config.'
        );
      const isFeatureEnabled = options?.enabled;
      if (!isFeatureEnabled)
        return Logger.warn(
          'push notification feature is not enabled yet, you can enable it from settings.'
        );
      ...
      if (permissionStatus === 'denied') return showBlockOverridePopup();
      if (permissionStatus === 'granted') return showBellConfiguration();
  ...
  async function showBlockOverridePopup() {
    ...
    console.log(
      ...
      settings?.popups?.blockOverride
    );
    if (!options?.enabled) return; //TODO
    const local_tenant_id = localStorage.getItem('tenant_id');
    const local_lead_id = localStorage.getItem('lead_id');
    ...
    try {
      if (local_tenant_id == null || local_lead_id == null) {
        return setGzLead({ vapid_config: {} }).then(() =>
  ...
  const requestRemoteNotificationPermission = async () => {
    const currentPermission = await window.Notification.requestPermission(
      (data) => {
  ...
  async function showNativePopup() {
    ...
    try {
      ...
      if (err === 'default') return handleNativePopupClose();
  ...
  function onPageView(settings) {
    return new Promise((resolve, reject) => {
      if (!settings) reject('The page view trigger has got an empty settings');
      ...
      const observeUrlChange = () => {
        ...
        const observer = new MutationObserver((mutations) => {
          mutations.forEach(() => {
            ...
            getUrlPaths != null
              ? (getExistingUrlFromLocalStorage = JSON.parse(getUrlPaths))
              : null;
            paths.add(pathname);
            getUrlPaths != null
              ? getExistingUrlFromLocalStorage.forEach((item) =>
                  paths.add(item)
                )
              : null;
            localStorage.setItem('urlPaths', JSON.stringify([...paths]));
            getUrlPaths != null
              ? (pathCount = JSON.parse(getUrlPaths).length)
              : null;
            if (pathCount + 1 >= pageView) resolve(true);
          });
        });
  ...
  function onScroll(options) {
    ...
    return new Promise((resolve, reject) => {
      if (!options) reject('The timeout trigger has got an empty settings');
      const { scroll } = options;
      if (scroll === undefined || isNaN(scroll))
        reject('The timeout trigger has got an invalid settings');
      if (scroll === undefined || isNaN(scroll)) return reject(false);
  ...
  function onTimeout(settings) {
    return new Promise((resolve, reject) => {
      if (!settings) reject('The timeout trigger has got an empty settings');
      ...
      const requestPopupTimeDelay = localStorage.getItem(
        'requestPopupTimeDelay'
      );
      const requestPopupTimeOut = localStorage.requestPopupTimeOut;
      if (requestPopupTimeDelay == null) {
        localStorage.setItem(
          'requestPopupTimeDelay',
          JSON.stringify(timeDelay)
        );
      }
      if (
        requestPopupTimeOut == null ||
        JSON.parse(requestPopupTimeDelay) !== timeDelay
      ) {
  ...
  function triggers(config) {
    switch (config.method) {
      ...
      default:
        throw new Error(
          'Push notification handler meet a unsupported trigger trigger settings ==>' +
  ...
  function blockOverrideOnTimeout(settings) {
    return new Promise((resolve, reject) => {
      ...
      if (timeDelay === undefined || isNaN(timeDelay))
        return reject('The timeout trigger has got an invalid settings');
      const blockOverrideTimeDelay = localStorage.getItem(
        'blockOverrideTimeDelay'
      );
      ...
      if (
        blockOverrideTimeOut == null ||
        JSON.parse(blockOverrideTimeDelay) !== timeDelay
      ) {
        const blockOverrideDate = new Date();
        blockOverrideDate.setSeconds(
          blockOverrideDate.getSeconds() + timeDelay
        );
  ...
  const showPermissionRequestingPopup = async (options) => {
    ...
    redirectFrame({
      iframe: Config.iFrames.pushNotificationPopupFrame,
      location: `/push/popup/${activePopupType}`,
    });
  };
  const showReminderPopup = async (options) => {
    ...
    redirectFrame({
      iframe: Config.iFrames.pushNotificationPopupFrame,
      location: `/push/popup/block-override`,
    });
  ...
  function sendEngagementPromptConfig({ frame, origin }) {
    const config = GlobalInstance.get('config');
    if (!config) throw new Error('Configuration is undefined, please check');
    const popup = config?.settings?.push_notification?.popups;
    if (!popup) throw new Error('Configuration is undefined, please check');
    // console.log({ popup });
    const message = {
      type: pushNotificationMessageTypes.POPUP_CONFIGURATION,
      data: popup,
    };
  ...
  const iframeStyles = {
    ...
    HIDDEN: `display:none;`,
    BELL_ICON:
      'display:block;position:fixed;z-index:99999999999999999;border:none;width:5rem;height:5rem;',
    BELL_ICON_FRAME:
      'display: block;position: fixed;z-index: 99999999999999999;border: none;width: 460px;height: 220px;',
    BLOCK_OVERRIDE:
      'display:block;position:fixed;z-index:99999999999999999;border:none;width:5rem;height:5rem;',
    BLOCK_OVERRIDE_FRAME:
      'display: block;position: fixed;z-index: 99999999999999999;border: none;width: 460px;height: 220px;',
    BELL_PUSH_CONFIGURATION:
      'display:block;position:fixed;z-index:99999999999999999;border:none;width:5rem;height:5rem;',
    BELL_PUSH_CONFIGURATION_FRAME:
      'display: block;position: fixed;z-index: 99999999999999999;border: none;width: 420px;height: 390px;',
  };
  ...
  function sendMessageToFrame({ iframe, message, origin = '*' }) {
    const frame =
      typeof iframe === 'string' ? document.getElementById(iframe) : iframe;
    if (!frame) throw new Error('Iframe not found, sending message has failed');
    try {
      frame.contentWindow?.postMessage(JSON.stringify(message), origin);
    } catch (err) {
      console.log(
        'Error occurred while sending message sending to iframe ( id | frame ) ==> ' +
          iframe,
        err
      );
      throw err;
  ...
  function sendMessage(params) {
    ...
    try {
      ...
      console.log(
        'Error occurred while sending message sending to window ( id | frame ) ==> ',
        window
      );
      throw err;
    }
  ...
  async function init() {
    ...
    try {
      ...
      if (!GlobalInstance.canInitialize()) throw Error('Already initialized.');
